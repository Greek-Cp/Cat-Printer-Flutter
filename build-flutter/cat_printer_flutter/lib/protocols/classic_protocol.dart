import '../core/interfaces.dart';

/// Classic Cat Printer Protocol (0x51 0x78 format)
/// Used by GB01, GB02, GT01, MX series
class ClassicProtocol implements IPrinterProtocol {
  @override
  String get protocolName => 'Classic Cat Printer';

  @override
  String get protocolVersion => '1.0';

  // CRC8 table for checksum calculation
  static const List<int> _crc8Table = [
    
  ];

  // Protocol constants
  static const int _magicByte1 = 0x51;
  static const int _magicByte2 = 0x78;
  static const int _endMarker = 0xff;

  // Data flow control patterns
  static const List<int> dataFlowPause = [
    0x51,
    0x78,
    0xae,
    0x01,
    0x01,
    0x00,
    0x10,
    0x70,
    0xff
  ];

  static const List<int> dataFlowResume = [
    0x51,
    0x78,
    0xae,
    0x01,
    0x01,
    0x00,
    0x00,
    0x00,
    0xff
  ];

  @override
  List<int> createCommand(String commandId, List<int> data) {
    final commandByte = _getCommandByte(commandId);
    return _makeCommand(commandByte, data);
  }

  @override
  Map<String, dynamic> parseResponse(List<int> response) {
    if (!isValidResponse(response)) {
      return {'error': 'Invalid response format'};
    }

    // Check for flow control responses
    if (_listEquals(response, dataFlowPause)) {
      return {'type': 'flow_control', 'action': 'pause'};
    }

    if (_listEquals(response, dataFlowResume)) {
      return {'type': 'flow_control', 'action': 'resume'};
    }

    // Parse standard response
    return {
      'type': 'standard',
      'command': response.length > 2 ? response[2] : null,
      'data':
          response.length > 6 ? response.sublist(6, response.length - 2) : [],
      'raw': response,
    };
  }

  @override
  List<String> getRequiredCharacteristics() {
    return [
      '0000ae01-0000-1000-8000-00805f9b34fb', // TX characteristic
      '0000ae02-0000-1000-8000-00805f9b34fb', // RX characteristic
    ];
  }

  @override
  bool isValidResponse(List<int> response) {
    if (response.length < 9) return false;
    return response[0] == _magicByte1 &&
        response[1] == _magicByte2 &&
        response[response.length - 1] == _endMarker;
  }

  /// Calculate CRC8 checksum
  int _calculateCrc8(List<int> data) {
    int crc = 0;
    for (int byte in data) {
      crc = _crc8Table[(crc ^ byte) & 0xFF];
    }
    return crc & 0xFF;
  }

  /// Create command with proper format
  List<int> _makeCommand(int commandByte, List<int> data, {List<int>? prefix}) {
    if (data.length > 0xFF) {
      throw ArgumentError('Command payload too big (${data.length} > 255)');
    }

    List<int> result = [];

    // Add prefix if provided (for newer printers)
    if (prefix != null) {
      result.addAll(prefix);
    }

    // Add command header
    result.addAll([_magicByte1, _magicByte2]);
    result.add(commandByte);
    result.add(0x00); // Reserved
    result.add(data.length); // Data length
    result.add(0x00); // Reserved

    // Add data
    result.addAll(data);

    // Add CRC8 and end marker
    result.add(_calculateCrc8(data));
    result.add(_endMarker);

    return result;
  }

  /// Get command byte from command ID
  int _getCommandByte(String commandId) {
    const commandMap = {
      'start_print': 0xa3,
      'start_print_new': 0xa3,
      'set_energy': 0xaf,
      'set_speed': 0xbd,
      'apply_energy': 0xbe,
      'set_quality': 0xa4,
      'drawing_mode': 0xbe,
      'get_device_state': 0xa3,
      'get_device_info': 0xa8,
      'update_device': 0xa9,
      'set_dpi_200': 0xa4,
      'start_lattice': 0xa6,
      'end_lattice': 0xa6,
      'retract_paper': 0xa0,
      'feed_paper': 0xa1,
      'draw_bitmap': 0xa2,
    };

    final commandByte = commandMap[commandId];
    if (commandByte == null) {
      throw ArgumentError('Unknown command ID: $commandId');
    }

    return commandByte;
  }

  /// Check if two lists are equal
  bool _listEquals(List<int> a, List<int> b) {
    if (a.length != b.length) return false;
    for (int i = 0; i < a.length; i++) {
      if (a[i] != b[i]) return false;
    }
    return true;
  }

  /// Convert integer to bytes with specified length
  List<int> intToBytes(int value, {int length = 1}) {
    if (value < 0) throw ArgumentError('Value must be non-negative: $value');

    int maxValue = (1 << (length * 8)) - 1;
    if (value > maxValue) {
      throw ArgumentError('Value too large: $value > $maxValue');
    }

    List<int> bytes = List.filled(length, 0);
    for (int i = 0; i < length; i++) {
      bytes[i] = (value >> (i * 8)) & 0xFF;
    }
    return bytes;
  }

  /// Reverse bits in a byte (for bitmap data)
  int reverseBits(int byte) {
    int result = 0;
    for (int i = 0; i < 8; i++) {
      if ((byte & (1 << i)) != 0) {
        result |= 1 << (7 - i);
      }
    }
    return result;
  }
}
